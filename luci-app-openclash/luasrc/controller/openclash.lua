...
function action_flush_fakeip_cache()
	...
	if is_running() then
  	...
  	state = luci.sys.exec(string.format('curl -sLk -m 3 -H "Content-Type: application/json" -H "Authorization: Bearer %s" -XPOST http://"%s":"%s"/cache/fakeip/flush', dase, daip, cn_port))
  end
...
local function dler_login()
	...
	if email and passwd then
		info = luci.sys.exec(string.format("curl -sLk -H 'Content-Type: application/json' -d '{\"email\":\"%s\", \"passwd\":\"%s\"}' -X POST https://dler.cloud/api/v1/login", email, passwd))
		if info then
		...
		if info and info.ret == 200 then
			...
			uci:commit("openclash")
			get_sub = string.format("curl -sLk -H 'Content-Type: application/json' -d '{\"access_token\":\"%s\"}' -X POST https://dler.cloud/api/v1/managed/clash -o %s", token, sub_path)
			...
			if sub_info and sub_info.ret == 200 then
				...
				for _,v in ipairs(sub_key) do
					...
					luci.sys.exec(string.format('curl -sLk -m 3 --retry 2 --user-agent "clash" "%s" -o "/etc/openclash/config/Dler Cloud - %s.yaml" >/dev/null 2>&1', sub_info[v], v))
				end
			end
			return info.ret
		else
...
local function dler_logout()
	...
	if token then
		info = luci.sys.exec(string.format("curl -sLk -H 'Content-Type: application/json' -d '{\"access_token\":\"%s\"}' -X POST https://dler.cloud/api/v1/logout", token))
		if info then
			info = json.parse(info)
		end
		if info and info.ret == 200 then
...
local function dler_info()
	...
	if token and email and passwd then
		get_info = string.format("curl -sLk -H 'Content-Type: application/json' -d '{\"email\":\"%s\", \"passwd\":\"%s\"}' -X POST https://dler.cloud/api/v1/information -o %s", email, passwd, path)
...
local function dler_checkin()
	...
	if token and email and passwd then
		info = luci.sys.exec(string.format("curl -sLk -H 'Content-Type: application/json' -d '{\"email\":\"%s\", \"passwd\":\"%s\", \"multiple\":\"%s\"}' -X POST https://dler.cloud/api/v1/checkin", email, passwd, multiple))
		if info then
			info = json.parse(info)
		end
		if info and info.ret == 200 then
...
function sub_info_get()
	...
	if filename and not is_start() then
		...
		if not sub_url then
			...
			info = luci.sys.exec(string.format("curl -sLkI -X GET -m 10 -w 'http_code='%%{http_code} -H 'User-Agent: Clash' '%s'", sub_url))
			if not info or tonumber(string.sub(string.match(info, "http_code=%d+"), 11, -1)) ~= 200 then
				info = luci.sys.exec(string.format("curl -sLkI -X GET -m 10 -w 'http_code='%%{http_code} -H 'User-Agent: Quantumultx' '%s'", sub_url))
			end
			if info then
...
function action_rule_mode()
	...
	if is_running() then
		...
		if not daip or not cn_port then return end
		info = json.parse(luci.sys.exec(string.format('curl -sLk -m 3 -H "Content-Type: application/json" -H "Authorization: Bearer %s" -XGET http://"%s":"%s"/configs', dase, daip, cn_port)))
		if info then
			mode = info["mode"]
		else
...
function action_switch_rule_mode()
	...
	if is_running() then
		...
		info = luci.sys.exec(string.format('curl -sLk -m 3 -H "Content-Type: application/json" -H "Authorization: Bearer %s" -XPATCH http://"%s":"%s"/configs -d \'{\"mode\": \"%s\"}\'', dase, daip, cn_port, mode))
		if info ~= "" then
...
function action_switch_log()
	...
	if is_running() then
		...
		if not daip or not cn_port then luci.http.status(500, "Switch Faild") return end
		info = luci.sys.exec(string.format('curl -sLk -m 3 -H "Content-Type: application/json" -H "Authorization: Bearer %s" -XPATCH http://"%s":"%s"/configs -d \'{\"log-level\": \"%s\"}\'', dase, daip, cn_port, level))
		if info ~= "" then
			luci.http.status(500, "Switch Faild")
		end
	else
...
function action_toolbar_show()
	...
	if pid and pid ~= "" then
		...
		local cn_port = cn_port()
		if not daip or not cn_port then return end
		traffic = json.parse(luci.sys.exec(string.format('curl -sLk -m 3 -H "Content-Type: application/json" -H "Authorization: Bearer %s" -XGET http://"%s":"%s"/traffic', dase, daip, cn_port)))
		connections = json.parse(luci.sys.exec(string.format('curl -sLk -m 3 -H "Content-Type: application/json" -H "Authorization: Bearer %s" -XGET http://"%s":"%s"/connections', dase, daip, cn_port)))
