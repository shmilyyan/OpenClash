...
local function dler_checkin()
	...
	if token and email and passwd and enable == "1" then
		checkin = string.format("curl -sLk -H 'Content-Type: application/json' -d '{\"email\":\"%s\", \"passwd\":\"%s\", \"multiple\":\"%s\"}' -X POST https://dler.cloud/api/v1/checkin -o %s", email, passwd, multiple, path)
